...
def getBoardData():
    ...
    try:
        ...
        sql+="from ("
        sql+="select * from Board where b_no <= "+b_no+" order by b_date desc"
        sql+=")"
        sql+=")"
...
# @app.route("/updateReply")
# def updateMyReply():
#     br_txt = request.args.get("br_txt")
#     br_b_no = request.args.get("br_b_no")
#     br_no = request.args.get("br_no")
#     con, cur= None, None
#     data = []
#     try:
#         con = connect("sollind/end1128@192.168.0.126:1521/xe")
# #         update Board_Reply set br_txt = 'test' where br_no = 26;
#         sql = "update Board_Reply set br_txt = '"+br_txt+"' where br_no = "+br_no
#         # db작업 총괄 객체
#         cur = con.cursor()
#         
#         # sql문 전송
...
#         sql = "select * "
#         sql += "from ("
#         sql += "select rownum as rn, br_no, br_txt, br_m_id, br_date, br_b_no "
#         sql += "from ("
#         sql += "select * "
#         sql += "from Board_Reply "
#         sql += "where br_b_no = " + br_b_no + " "
#         sql += "order by br_date desc))where rn >= 0 and rn <= 10"
#         # db작업 총괄 객체
#         cur = con.cursor()
#         
#         # sql문 전송
#         cur.execute(sql)
#         db = {}
#         for rn, no, txt, m_id, date, b_no in cur:
#             db = {"rn": rn, "br_no": no, "br_txt": txt, "br_m_id": m_id, "br_date": date, "br_b_no": b_no}
#             data.append(db)
#     except Exception as e:
#         print(e)
#         pass
...
#         con = connect("sollind/end1128@192.168.0.126:1521/xe")
# #         update Board_Reply set br_txt = 'test' where br_no = 26;
#         sql = "delete from Board_Reply where br_no = "+br_no
#         # db작업 총괄 객체
#         cur = con.cursor()
#         
#         # sql문 전송
